...
# /tmp is a tmpfs in RAM on Buildroot Creality (small, ~50-100 MB).
# moonraker.tar.gz is ~23 MB and Buildroot RAM gets crowded fast → use disk.
TAR_TMP="/usr/data/.tmp_install/moonraker.tar.gz"
...
step_download_tar() {
    ...
    # /tmp is a tmpfs in RAM on Buildroot Creality and too small for 23 MB.
    # We use /usr/data/.tmp_install/ which is on the real flash partition.
    TMP_DIR=$(dirname "$TAR_TMP")
...
step_extract() {
    ...
    rm -f "$TAR_TMP"
    rmdir "$(dirname "$TAR_TMP")" 2>/dev/null || true

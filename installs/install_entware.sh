...
# Nebula Pad uses Ingenic X2000 SoC (XBurst II = MIPS32r2 little-endian)
# All Creality printers with Nebula Pad share this architecture (K1, K1 Max, E5M)
...
# Note: pas de virtualenv (n'existe plus, sera installé via pip si besoin),
#       pas de libsodium-dev (les headers sont fournis avec le paquet de base
#       sur Entware, contrairement à Debian),
#       pas de libssl (s'appelle libopenssl sur Entware, déjà installé en
#       dépendance de curl/wget-ssl/python3-openssl),
#       pas de gcc/make (143 Mo requis, pas la place sur /opt qui partage
#       /overlay avec le reste du système. Les wheels Python suffisent
#       pour Klipper/Moonraker dans 99% des cas).
BASE_PACKAGES="\
...
step_precheck() {
    ...
    if [ "$ARCH" != "mips" ]; then
        log_warn "Expected mips (Ingenic X2000), got '$ARCH'"

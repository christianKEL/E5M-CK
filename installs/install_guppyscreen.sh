...
step_start_guppy() {
    ...
    if pgrep -x "guppyscreen" >/dev/null 2>&1; then
        log_ok "GuppyScreen is running (pid: $(pgrep -x guppyscreen))"
    else
        log_error "GuppyScreen failed to start"
        log_warn "Check logs and configuration. Continuing for now."
        log_warn "You can try manually: $GUPPY_BIN &"
    fi
}
...
step_disable_creality() {
    ...
    p "    ${WHITE}• Removes 'shakehands' spam in klippy.log${NC}"
    ...
    if confirm "Disable all Creality services?"; then
        ...
        for proc in master-server app-server display-server Monitor; do
            if pgrep -x "$proc" >/dev/null 2>&1; then
                log_action "Killing $proc..."
        ...
        log_info "Keeping Creality services (minimal mode: disable Monitor + display-server only)"
        ...
        # Kill running processes (they'll respawn from S99start_app, then fail
        # because binaries are renamed — that's the intent)
        for proc in display-server Monitor; do
            if pgrep -x "$proc" >/dev/null 2>&1; then
                log_action "Killing $proc..."
                killall -9 "$proc" 2>/dev/null || true
            fi

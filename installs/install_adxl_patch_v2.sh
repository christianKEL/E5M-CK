...
#     curl -sSL https://raw.githubusercontent.com/christianKEL/E5M-CK/main/installs/install_adxl_patch.sh | sh
#     curl -sSL https://raw.githubusercontent.com/christianKEL/E5M-CK/main/installs/install_adxl_patch.sh | sh -s -- status
#     curl -sSL https://raw.githubusercontent.com/christianKEL/E5M-CK/main/installs/install_adxl_patch.sh | sh -s -- update
#     curl -sSL https://raw.githubusercontent.com/christianKEL/E5M-CK/main/installs/install_adxl_patch.sh | sh -s -- uninstall
#
#   Or download once and run locally:
#     wget https://raw.githubusercontent.com/christianKEL/E5M-CK/main/installs/install_adxl_patch.sh
#     sh install_adxl_patch.sh install
# ═══════════════════════════════════════════════════════════════════
...
preflight() {
    ...
    case "$REPO_RAW" in
        *CHANGE_ME*)
            die "REPO_RAW is not configured. Edit install_adxl_patch.sh and set the GitHub raw URL, or run with REPO_RAW=https://... sh install_adxl_patch.sh"
            ;;
    esac
}
...
cfg_patch() {
    ...
    log "Appending [accel_chip_proxy] + [resonance_tester] to printer.cfg"
    cat >> "$PRINTER_CFG" <<EOF

$CFG_MARKER_BEGIN
# Auto-added by adxl345-creality-bridge install.sh
...
cmd_help() {
    cat <<EOF
${BOLD}install.sh — adxl345-creality-bridge installer${RESET}

...
## 4. Expected warnings — and the Klipper-side suppression patch

The bridge works by talking to the Creality 2023 firmware using its **original protocol**. Klipper mainline 2026 expects newer command names for some features. When the MCU doesn't advertise them, Klipper emits a deprecated-feature warning on every startup. Since we **deliberately** keep the Creality firmware (cf §1.1), reflashing isn't an option — the warnings would be permanent visual noise in Fluidd.

### 4.1 Warnings observed

After bridge installation, Klipper emits warnings of this form:

```
MCU 'nozzle_mcu' has deprecated code (it is missing feature 'spi_set_sw_bus').
Recompiling and flashing is recommended (MCU version '341a2c18-dirty-20230717_152940-cxsw-virtual-machine',
host version 'v0.13.0-642-g1b1da9331-dirty').
```

And similarly for `STEPPER_STEP_BOTH_EDGE`.

Both are **functionally harmless** but visually persistent. Detail per feature:

#### `spi_set_sw_bus`

Generated by `klippy/extras/bus.py:97`:

```python
if self.mcu.try_lookup_command("spi_set_sw_bus oid=%c miso_pin=%u mosi_pin=%u "
                               "sclk_pin=%u mode=%u pulse_ticks=%u"):
    # use the command
else:
    configfile.deprecate_mcu_code(self.mcu, 'spi_set_sw_bus')
    # fall back to old form (still works)
```

The Creality 2023 firmware **does** have a `spi_set_sw_bus` command, but with a slightly different parameter signature (probably without `pulse_ticks` or with different parameter ordering). `try_lookup_command` fails to match the modern signature, so the warning fires. The fall-back code path then uses the older signature successfully.

This is precisely the path the ADXL345 bridge needs: any rewrite to use `config_software_spi` would require reflashing the nozzle MCU, which we explicitly avoid.

#### `STEPPER_STEP_BOTH_EDGE`

A different deprecation related to step pulse handling. The Creality firmware doesn't support both-edge stepping; we compensate with `step_pulse_duration: 0.000000501` in every `[stepper_*]` block. The behavior is identical to both-edge stepping but uses two pulses per step instead of one.

### 4.2 Suppression patch

Both warnings are suppressed by patching `klippy/configfile.py`:
...
The early `return` short-circuits the warning generation. Other deprecation warnings (for features we don't know about and might genuinely care about) are still emitted normally.

### 4.3 Why not just ignore the warning visually?

Three reasons we patch rather than ignore:

1. **A persistent red banner in Fluidd hides real warnings.** If a future genuine deprecation appears, we want to see it.
2. **Documentation discipline.** A user (or future-me) looking at the printer should see "zero warnings" as the normal state, not "two warnings, both intentional".
3. **The cost is trivial.** A 5-line patch to one file, easy to re-apply after Klipper updates.

### 4.4 Maintenance after Klipper update

Klipper updates may overwrite `configfile.py`. The backup is preserved as `configfile.py.bak.before_e5m_ck` (and `.bak.before_spi_warning.<timestamp>` for the most recent revision). To re-apply after an update:

```bash
# Compare current vs known-patched state
diff /usr/data/klipper/klippy/configfile.py \
     /usr/data/klipper/klippy/configfile.py.bak.before_e5m_ck

# If the patch is missing, re-apply (use the Python script in installs/
# or run sed/manual edit on the deprecate_mcu_code function)
```

A future enhancement to `install_adxl_patch_v2.sh` could detect a missing patch and offer to re-apply it.

---

## 5. Installer — `install_adxl_patch_v2.sh`
...
## 7. Failure modes encountered (and resolved)

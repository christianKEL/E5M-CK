# adxl345_creality — ADXL345 driver speaking the stock Creality 2023 dialect.
#
# Purpose
# -------
# The ADXL345 on the Ender 5 Max's print head is wired into the stock
# nozzle-MCU's SPI bus. Mainline Klipper's klippy/extras/adxl345.py
# cannot drive it because the stock firmware's query/response protocol
# pre-dates several mainline changes:
#   - 3-arg `query_adxl345 oid clock rest_ticks` vs mainline's 2-arg form
#   - 5-byte sample packing with 0x60 Z-mask vs mainline's 6-byte / 0xe0
#   - callback registered on MCU._serial vs the old MCU object location
#
# This module is a port of the Creality ADXL345 class that reuses
# mainline's AccelCommandHelper and BatchBulkHelper (so resonance_tester
# and shaper_calibrate work unchanged) while keeping the stock-dialect
# wire format on the I/O side.
#
# Users do NOT instantiate it directly with a section header — there is
# no [adxl345_creality] section. The driver is reached through
# accel_chip_proxy (sibling module in this directory), which is what
# printer.cfg's [accel_chip_proxy] section dispatches.
#
# Why we don't reflash the nozzle MCU
# -----------------------------------
# Reflashing breaks our factory_reset rollback path. The stock firmware
# is a precondition for restoring the original Creality experience. See
# docs/operations/factory_reset.md and installs/creality_kill.sh.
#
# Operational quirk (documented for the next person who reads this)
# -----------------------------------------------------------------
# Raw acceleration magnitudes are wrong: the chip reports ~3.66 g at
# rest instead of 1.0 g, due to sign-extension in the Creality unpacking.
# This does NOT affect input-shaper output because resonance analysis is
# dynamics-based (FFT of acceleration deltas), not absolute magnitude.
# It would matter only if anyone tried to use this chip as an absolute
# tilt or g-force sensor — don't.
#
# Klipper master compatibility (verified on commit bd09e0170, May 2026)
# ---------------------------------------------------------------------
# Pin-for-pin port of the Creality klippy/extras/adxl345.py at firmware
# commit (approx) v0.13.0-628 onto Klipper master's 2026 host
# architecture. If either side refactors AccelCommandHelper or the
# BatchBulkHelper contract, this module will need re-porting.
#
...
class ADXL345Creality:
    def __init__(self, config, chip_kind='adxl345', register_commands=True):
        ...
        # Mainline 2026: register_response moved from MCU to MCU_serial.
        # The adxl345_data wire format is still oid + sequence + data
        # (Creality 2023 firmware), so we register the same callback
        # but via the serial layer.
        mcu._serial.register_response(
            self._handle_adxl345_data, "adxl345_data", oid)
    ...
    def _extract_samples(self, raw_samples):
        ...
        for params in raw_samples:
            ...
            for i in range(len(d) // BYTES_PER_SAMPLE):
                ...
                # Z spans 13 bits across two carrier bytes:
                #   bits  0-7   in zlow
                #   bits  8-10  in xzhigh & 0xe0 (3 bits, shifted to position)
                #   bits 11-12  in yzhigh & 0x60 (2 bits — NOT 0xe0!
                #                  bit 7 of yzhigh is Y sign-extension)
                # Sign bit of Z is bit 12 (0x1000), which lands at the
                # 0x2000 position when (yzhigh & 0x40) << 7 — subtract it
                # for two's-complement extension.
                rz = ((zlow | ((xzhigh & 0xe0) << 3) | ((yzhigh & 0x60) << 6))
                      - ((yzhigh & 0x40) << 7))
                raw_xyz = (rx, ry, rz)
...
class ADXL345(ADXL345Creality):
    def __init__(self, config, register_commands=False):

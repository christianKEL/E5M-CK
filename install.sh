...
show_main_menu() {
    ...
    p "       ${DIM}Runs all steps 0 through 10 sequentially${NC}"
    p ""
    p "  ${BG_RED}${WHITE}${BOLD} 2 ${NC}  ${WHITE}${BOLD}Manual step-by-step installation${NC}"
    p "       ${DIM}Choose individual steps to run${NC}"
    p ""
    p "  ${BG_RED}${WHITE}${BOLD} 3 ${NC}  ${WHITE}${BOLD}Configure BTT Eddy offsets${NC} ${GRAY}(post-install)${NC}"
    p "       ${DIM}Set x_offset and y_offset, recompute bed_mesh${NC}"
    p ""
    p "  ${BG_RED}${WHITE}${BOLD} 4 ${NC}  ${WHITE}${BOLD}Eddy calibration assistant${NC} ${GRAY}(step-by-step guide)${NC}"
    p "       ${DIM}Full guided calibration (requires Fluidd open)${NC}"
    p ""
    p "  ${BG_BLACK}${WHITE} q ${NC}  ${GRAY}Quit${NC}"
    p ""
    printf "  ${WHITE}Your choice [1/2/3/4/q]: ${NC}"
    read MAIN_CHOICE
}

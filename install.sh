...
eddy_calibration_assistant() {
    ...
    p "  ${WHITE}  ${BR_GREEN}${BOLD}1.${NC} ${BR_GREEN}G28${NC}                              ${DIM}# home all axes${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}2.${NC} ${BR_GREEN}CENTER_TOOLHEAD${NC}                  ${DIM}# move to center${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}3.${NC} ${BR_GREEN}BED_MESH_CLEAR${NC}                   ${DIM}# clear any mesh${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}4.${NC} ${WHITE}Place A4 paper between nozzle and bed${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}5.${NC} ${WHITE}Using Fluidd Z arrows with ${BOLD}0.1mm step${NC}${WHITE}:${NC}"
    ...
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}6.${NC} ${BR_GREEN}SET_KINEMATIC_POSITION Z=0${NC}       ${DIM}# declare this is Z=0${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}7.${NC} ${BR_GREEN}G1 Z1 F300${NC}                       ${DIM}# lift 1mm${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}8.${NC} ${WHITE}REMOVE the paper sheet from the bed${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}9.${NC} ${BR_GREEN}PROBE_EDDY_CURRENT_CALIBRATE CHIP=btt_eddy${NC}"
    p "  ${WHITE}     ${DIM}(Klipper will run its own sequence — wait patiently)${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}10.${NC} ${WHITE}When Klipper prompts ${BOLD}TESTZ / ACCEPT${NC}${WHITE}:${NC}"
    p "  ${WHITE}      Replace paper, use ${BR_GREEN}TESTZ Z=-0.1${NC}${WHITE} until paper catches,${NC}"
    p "  ${WHITE}      then send ${BR_GREEN}ACCEPT${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}11.${NC} ${WHITE}Wait for full calibration (~2 minutes, many moves)${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}12.${NC} ${BR_GREEN}SAVE_CONFIG${NC}                     ${DIM}# Klipper will restart${NC}"
    p ""
...
mesh_min_x = max(-x, 0) + SCAN_OVERSHOOT
mesh_max_x = POS_MAX_X - max(x, 0) - SCAN_OVERSHOOT
mesh_min_y = max(-y, 0)
mesh_max_y = POS_MAX_Y - max(y, 0)

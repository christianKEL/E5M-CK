...
show_main_menu() {
    ...
    p "       ${DIM}Runs all steps 0 through 11 sequentially${NC}"
    p ""
    ...
    p ""
    p "  ${DIM}For Eddy offsets and calibration, run the separate eddy.sh tool:${NC}"
    p "  ${DIM}  wget --no-check-certificate ${UNDER}${GITHUB_RAW}/eddy.sh${NC}${DIM} -O /tmp/eddy.sh && sh /tmp/eddy.sh${NC}"
    p ""
...
GITEXCL
    ...
    # Mark ALL files modified by Helper Script installs as assume-unchanged
    git update-index --assume-unchanged files/guppy-screen/guppy-update.sh 2>/dev/null || true
    # Also add files potentially modified during install
    for f in $(git status --porcelain 2>/dev/null | awk '{print $2}'); do
        git update-index --assume-unchanged "$f" 2>/dev/null || true
        log_action "assume-unchanged: $f"
    done
    log_action "Helper Script repo is clean for Update Manager"
...
# NOTE: [safe_z_home] is NOT used because [homing_override] is present
# in homing.cfg and they are mutually exclusive. The homing_override
# handles the "move to center before Z probe" behavior manually.
...
gcode:
  ...
  {% endif %}
  RESPOND TYPE=command MSG="Move bed until nozzle catches paper"
...
MACROEOF
    ...
    # Write Python script to temp file (more robust than heredoc on busybox)
    cat > /tmp/add_include.py << 'PYTMP'
import sys
...
try:
    ...
    pass
PYTMP
    python3 /tmp/guppy_visible.py
    rm -f /tmp/guppy_visible.py
...
eddy_calibration_assistant() {
    ...
    p "${BR_RED}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"
    p ""
    p "  ${WHITE}Before calibrating the Eddy probe, verify the printer is ready.${NC}"
    p ""
    p "  ${YELLOW}${BOLD}NOTE: CAL_BED_Z_TILT cannot run yet — it requires a calibrated${NC}"
    p "  ${YELLOW}${BOLD}Eddy probe, which is what we are about to set up.${NC}"
    p "  ${YELLOW}You can run CAL_BED_Z_TILT AFTER this assistant completes.${NC}"
    p ""
    ...
    if ! _confirm_yes "XY homing works correctly and toolhead reaches center"; then
    ...
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}4.${NC} ${WHITE}Using Fluidd ${BOLD}Z arrows${NC}${WHITE}, move bed ${BR_RED}${BOLD}DOWN${NC}${WHITE} by ${BOLD}20mm${NC}"
    p "  ${WHITE}     ${DIM}Click the Z arrow that LOWERS the bed (moves it away from nozzle)${NC}"
    p "  ${WHITE}     ${DIM}Use 10mm step, click twice. Final gap should be ~20mm.${NC}"
    p ""
    ...
    p "  ${WHITE}${BOLD}Instructions (in Fluidd console):${NC}"
    p ""
    p "  ${YELLOW}${BOLD}  Important: after SAVE_CONFIG the bed may be close to the nozzle.${NC}"
    p "  ${YELLOW}${BOLD}  Use SET_KINEMATIC_Z_200 trick to move bed safely before G28.${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}1.${NC} ${BR_GREEN}G28 X Y${NC}                          ${DIM}# home X Y only${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}2.${NC} ${BR_GREEN}CENTER_TOOLHEAD${NC}                  ${DIM}# move to center${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}3.${NC} ${BR_GREEN}SET_KINEMATIC_Z_200${NC}              ${DIM}# fake Z=200${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}4.${NC} ${WHITE}Using Fluidd Z arrows, move bed ${BR_RED}${BOLD}DOWN${NC}${WHITE} ~20mm${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}5.${NC} ${BR_GREEN}G28${NC}                              ${DIM}# now full home works${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}6.${NC} ${BR_GREEN}CENTER_TOOLHEAD${NC}                  ${DIM}# re-center${NC}"
    p "  ${WHITE}  ${BR_GREEN}${BOLD}7.${NC} ${BR_GREEN}G1 Z0 F300${NC}                       ${DIM}# go to Z=0${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}8.${NC} ${WHITE}Place A4 paper between nozzle and bed${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}9.${NC} ${WHITE}Check paper resistance:${NC}"
    p "  ${WHITE}     ${BR_GREEN}• Paper catches slightly${NC} ${WHITE}→ calibration OK, go to step 11${NC}"
    p "  ${WHITE}     ${YELLOW}• Paper moves freely${NC} ${WHITE}→ nozzle too high, do step 10${NC}"
    p "  ${BR_RED}     • Paper is stuck / crushed${NC} ${WHITE}→ nozzle too low, do step 10${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}10.${NC} ${WHITE}If adjustment needed, use ${BOLD}babystepping${NC}${WHITE}:${NC}"
    ...
    p "  ${WHITE}      ${DIM}Repeat until perfect, then ${BR_GREEN}Z_OFFSET_APPLY_PROBE${NC}${DIM} + ${BR_GREEN}SAVE_CONFIG${NC}"
    p ""
    p "  ${WHITE}  ${BR_GREEN}${BOLD}11.${NC} ${WHITE}Calibration validated${NC}"
    p ""
...
step11_final_cleanup() {
    log_step "11" "Final cleanup (Update Manager)"

    log_info "Cleaning Klipper repo state..."
    cd /usr/data/klipper 2>/dev/null || true
    for f in $(git status --porcelain 2>/dev/null | awk '{print $2}'); do
        git update-index --assume-unchanged "$f" 2>/dev/null || true
    done
    log_action "Klipper: all modified files marked assume-unchanged"

    log_info "Cleaning Helper Script repo state..."
    cd /usr/data/helper-script 2>/dev/null || true
    for f in $(git status --porcelain 2>/dev/null | awk '{print $2}'); do
        git update-index --assume-unchanged "$f" 2>/dev/null || true
    done
    log_action "Helper Script: all modified files marked assume-unchanged"

    log_info "Refreshing Moonraker Update Manager..."
    curl -s -X POST "http://localhost:7125/machine/update/refresh" >/dev/null 2>&1 || true
    log_action "Update Manager refreshed"

    log_ok "All repositories clean for Update Manager"
}
...
main() {
    ...
    # Otherwise, show interactive menu (with loop to return to main menu)
    show_banner
    show_disclaimer
    show_prerequisites
    ...
    while true; do
        ...
        case $MAIN_CHOICE in
            1)
                ...
                run_all
                p ""
                pause_user "Press ENTER to return to the main menu..."
                ;;
            2)
                menu_loop
                # menu_loop has its own "back to menu" logic, returns here when user quits it
                ;;
            3)
                clear
                show_banner
